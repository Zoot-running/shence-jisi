  ...
  const spawner = {
    spawn(work, opts) {
      ...
      const report = (async () => {
        ...
        const anchor = process.env.DSH_ANCHOR_TEST === "1" && opts.model === "deepseek-v4-pro" ? { persona: "You are a helpful software engineer assistant." } : {};
...
function apply(ctx, config = {}) {
  ...
  const priceTable = config.priceTable ?? {
    ...
    "glm-5.1": { input: 8, output: 28, cacheRead: 2 },
    // ESTIMATE(比照 glm-5.3); 平台网关是否支持未验证——手动显式派单前先验证
